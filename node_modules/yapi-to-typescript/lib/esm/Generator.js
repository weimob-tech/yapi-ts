...
import { Method, RequestBodyType } from './types';
import { exec } from 'child_process';
import { getCachedPrettierOptions, getNormalizedRelativePath, getRequestDataJsonSchema, getResponseDataJsonSchema, jsonSchemaToType, sortByWeights, throwError } from './utils';
...
export class Generator {
  ...
  async generate() {
    ...
    await Promise.all(this.config.map(async (serverConfig, serverIndex) => {
      const projects = serverConfig.projects.reduce((projects, project) => {
        projects.push(...castArray(project.token).map(token => ({ ...project,
          token: token
        })));
        return projects;
      }, []);
      return Promise.all(projects.map(async (projectConfig, projectIndex) => {
        ...
        await Promise.all(projectConfig.categories.map(async (categoryConfig, categoryIndex) => {
          // 分类处理
          // 数组化
          let categoryIds = castArray(categoryConfig.id); // 全部分类
          ...
          const codes = (await Promise.all(categoryIds.map(async (id, categoryIndex2) => {
            ...
            interfaceList = interfaceList.map(interfaceInfo => {
              ...
              const _interfaceInfo = isFunction(syntheticalConfig.preproccessInterface) ? syntheticalConfig.preproccessInterface(cloneDeepFast(interfaceInfo), changeCase) : interfaceInfo;
  ...
  async write(outputFileList) {
    return Promise.all(Object.keys(outputFileList).map(async outputFilePath => {
      ...
      const prettyOutputContent = prettier.format(rawOutputContent, { ...(await getCachedPrettierOptions()),
      ...
      const outputContent = `${dedent`
          ...
          /* prettier-ignore-end */
        `}\n`;
      await fs.outputFile(outputFilePath, outputContent); // 如果要生成 JavaScript 代码，
  ...
  async tsc(file) {
    return new Promise(resolve => {
      ...
      exec(`${command} --target ES2019 --module ESNext --jsx preserve --declaration --esModuleInterop ${file}`, {
  ...
  async generateInterfaceCode(syntheticalConfig, interfaceInfo, categoryUID) {
    ...
    const requestHookName = syntheticalConfig.reactHooks && syntheticalConfig.reactHooks.enabled ? isFunction(syntheticalConfig.reactHooks.getRequestHookName) ?
    /* istanbul ignore next */
    ...
    return dedent`
      ...
      return res;
              ...
              requestBodyType: RequestBodyType.${extendedInterfaceInfo.method === Method.GET ? RequestBodyType.query : extendedInterfaceInfo.req_body_type
    /* istanbul ignore next */
